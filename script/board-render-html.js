...
function renderContactAvatarHTML(contact) {
  return `<div class="avatar" title="${contact.name}">${getInitials(contact.name)}</div>`;
}

/**
 * Renders a single Counter contact avatar.
 * @param {Contact} contact
 * @returns {string}
 */
function renderContactAvatarHTMLMax(remaining) {
  return `<div class="avatar" style="background-color: #ffffff; color: #2a3647;">
          +${remaining}
        </div>`;
}
...
function taskPopupEditMode(task, id) {
  return `<div class="task-popup edit-mode">
...
function buildEditForm(task, id) {
  return `<form id="editTaskForm" onsubmit="saveEditedTask('${id}'); return false;">
...
function buildCategorySelect(current) {
  return `<select name="priorityCategory" id="priorityCategory">
...
function loadAddTaskFormIntoOverlay() {
  return (document.getElementById("addTaskFormContainer").innerHTML =
      ...
      <main class="main-content">
        ...
        <section class="card">
          <div class="field">
            <label class="field-label">Priority</label>
            <div class="button-priority">              
              <input type="radio" id="prio-urgent" name="priority" value="urgent" class="prio-input d-none" ${isChecked(task.priority, "urgent")}>
              <label for="prio-urgent" class="btn-priority btn-priority-flex">
              ...
              <input type="radio" id="prio-medium" name="priority" value="medium" class="prio-input d-none" ${isChecked(task.priority, "medium")}>
              ...
              <input type="radio" id="prio-low" name="priority" value="low" class="prio-input d-none" ${isChecked(task.priority, "low")}>
              <label for="prio-low" class="btn-priority btn-priority-flex">
...
function getProgressSubtask(stats) {
  return `<div class="progress-scale">
...
function renderTaskSubTaskDetailsHTML(subtask, i, taskId) {
  return `<div class="subtask-item">
        <input class="checkbox-input-subtask" type="checkbox" id="subtask-${taskId}-${i}" onchange="toggleSubtask('${taskId}', ${i})" ${subtask.state === "check" ? "checked" : ""}>
