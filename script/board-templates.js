...
function renderTaskContact(contacts = [], maxVisible = 4) {
  ...
  for (let i = 0; i < checkedContacts.length; i++) {
    if (i < maxVisible) {
      ...
      html += renderContactAvatarHTMLMax(remaining);
    }
  }
  return html;
}
...
function buildCardProgress(task) {
  ...
  if (!stats) {return getProgressSubtaskZero();}
  return getProgressSubtask(stats);
}

/**
 * Returns "checked" if the current value matches the target value.
 * @param {string} current
 * @param {string} target
 * @returns {string}
 */
function isChecked(current, target) {
